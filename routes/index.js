...
exports.createRoutes = function(app_ref) {
    ...
    app.post('/logout', function(req, res) {
        req.session.destroy((function(err) {
            if(req.user.logged_in)
                delete app.db.connected_users[req.user._id];
    ...
    app.io.route('player_page_connected', function(req) {
        // req is a socket
        try {
            if (req.user.logged_in && req.user) {
                app.db.connected_users[req.user._id] = app.db.getUserPlaylist(req.user._id);
            }
        } catch (e) {
            console.log(e);
        }
    ...
    util.getip(function(ip) {
        ...
        var sendHome = function() {
            ...
            res.render((md.mobile() ? 'mobile' : 'index'), {
                ...
                demo: config.demo,
                user: user,
                log: req.user ? req.user.isAdmin : false,
            });
        };
...
function downloadPlaylist(req, res) {
    var db = app.db.playlists;
    if (req.user.logged_in) {
        db = app.db.connected_users[req.user._id];
    }
    db.findOne({
        _id: req.params.id
...
function returnPlaylists(req) {
    //get user playlists, logged in
    var db = app.db.playlists;
    if (req.user.logged_in) {
        db = app.db.connected_users[req.user._id];
    }
    // send the playlists back to the user
    get_playlists(db, function(playlists) {
...
function get_playlists(db, callback) {
    db.find({}).sort({
        title: 1
...
function createPlaylist(req) {
    ...
    db.insert(plist, function(err, doc) {
